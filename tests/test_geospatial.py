...
def test_interpolate_station_field_unknown_method():
    with pytest.raises(ValueError):
        interpolate_station_field(
            obs_lons=np.array([0.0]),
            obs_lats=np.array([0.0]),
...
def make_layer(data):
    ...
    return RasterLayer(data=data, lats=lats, lons=lons, name="t", crs=CRS(epsg=4326))
